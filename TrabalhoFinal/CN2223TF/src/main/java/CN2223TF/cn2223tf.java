...
//                     break;
    ...
    private static boolean verifyIfBucketExists() throws Exception {
        String[] buckets = storageOperations.listBuckets(PROJECT_ID);
    ...
    private static BlobId UploadPhoto(BufferedReader reader) throws Exception {
        //se não houver o bucket para criar
        boolean haveBucket = verifyIfBucketExists();
        System.out.println(haveBucket);
        if(!haveBucket) storageOperations.createBucket(bucketName);
        //upload da foto
        storageOperations.listBuckets(PROJECT_ID);
        System.out.println("Introduza o nome do ficheiro: ");
        String fileName = reader.readLine();
        File file = new File(fileName);
        BlobId blob = storageOperations.uploadBlobToBucket(bucketName, fileName, file.getAbsolutePath());
        return blob;
    }
